...
class Card extends JPanel{
	...
	private void drawSuit(Graphics2D g, String suit, Color color){
		g.setColor(color) ;
		g.drawString(suit, _location.x + x_offset + 1, _location.y + y_offset + 13) ;
		g.drawString(suit, _location.x + x_offset + (CARD_WIDTH - 31), _location.y + CARD_HEIGHT - 18) ;
	}

	private void drawValue(Graphics2D g, String value){
		g.drawString(value, _location.x + x_offset + 1, _location.y + y_offset) ;
		g.drawString(value, _location.x + x_offset + (CARD_WIDTH - 30), _location.y + y_offset + CARD_HEIGHT - 25) ;
	}
	...
	public void paintComponent(Graphics g){
		...
		// DRAW THE CARD SUIT AND VALUE IF OPEN
		if (_faceup){
			...
			// DRAW THE BACK OF THE CARD IF NOOPEN
